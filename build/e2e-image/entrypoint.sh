...
echo "consul lock -child-exit-code=true -timeout 1h -verbose $TEST_CLUSTER_NAME '/root/e2e.sh "$FEATURES" "$REGISTRY"'"
consul lock -child-exit-code=true -timeout 1h -verbose $TEST_CLUSTER_NAME '/root/e2e.sh "'$FEATURES'" "'$REGISTRY'"'
